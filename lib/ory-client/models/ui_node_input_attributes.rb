...
module OryClient
  ...
  class UiNodeInputAttributes < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OryClient::UiNodeInputAttributes`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      autocomplete_validator = EnumAttributeValidator.new('String', ["email", "tel", "url", "current-password", "new-password", "one-time-code", "username", "username webauthn"])
      ...
      node_type_validator = EnumAttributeValidator.new('String', ["input"])
    ...
    def autocomplete=(autocomplete)
      validator = EnumAttributeValidator.new('String', ["email", "tel", "url", "current-password", "new-password", "one-time-code", "username", "username webauthn"])
    ...
    def node_type=(node_type)
      validator = EnumAttributeValidator.new('String', ["input"])
    ...
    def hash
      [autocomplete, disabled, label, maxlength, name, node_type, onclick, onclick_trigger, onload, onload_trigger, options, pattern, required, type, value].hash
    end
