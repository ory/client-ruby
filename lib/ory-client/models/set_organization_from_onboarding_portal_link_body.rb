...
module OryClient
  class SetOrganizationFromOnboardingPortalLinkBody < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OryClient::SetOrganizationFromOnboardingPortalLinkBody` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OryClient::SetOrganizationFromOnboardingPortalLinkBody`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [kratos_selfservice_methods_oidc_config_providers, kratos_selfservice_methods_saml_config_providers, revision_id, scim_clients].hash
    end
