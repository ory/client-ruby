...
module OryClient
  ...
  class LoginFlowTestContext < ApiModelBase
    attr_accessor :debug_payload
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OryClient::LoginFlowTestContext` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OryClient::LoginFlowTestContext`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'debug_payload')
        self.debug_payload = attributes[:'debug_payload']
      end
    ...
    def hash
      [debug_payload, provider_id].hash
    end
