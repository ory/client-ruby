...
module OryClient
  class ListWorkspaceProjects < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OryClient::ListWorkspaceProjects` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OryClient::ListWorkspaceProjects`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def next_page=(next_page)
      if next_page.nil?
        fail ArgumentError, 'next_page cannot be nil'
      end
    ...
    def hash
      [has_next_page, next_page, projects].hash
    end
