...
module OryClient
  ...
  class TokenPaginationRequestParameters < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OryClient::TokenPaginationRequestParameters` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OryClient::TokenPaginationRequestParameters`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@page_size.nil? && @page_size > 500
        invalid_properties.push('invalid value for "page_size", must be smaller than or equal to 500.')
      end
    ...
    def valid?
      ...
      return false if !@page_size.nil? && @page_size > 500
    ...
    def page_size=(page_size)
      ...
      if page_size > 500
        fail ArgumentError, 'invalid value for "page_size", must be smaller than or equal to 500.'
      end
