...
module OryClient
  class NormalizedProjectRevisionTokenizerTemplate < ApiModelBase
    # Claims mapper URL
    attr_accessor :claims_mapper_url
    ...
    def valid?
      ...
      return false if !@ttl.nil? && @ttl !~ Regexp.new(/^([0-9]+([.][0-9]+)?(ns|us|µs|ms|s|m|h))+$/)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] subject_source Object to be assigned
    def subject_source=(subject_source)
      validator = EnumAttributeValidator.new('String', ["id", "external_id"])
      unless validator.valid?(subject_source)
        fail ArgumentError, "invalid value for \"subject_source\", must be one of #{validator.allowable_values}."
      end
      @subject_source = subject_source
    end
    ...
    def ttl=(ttl)
      ...
      pattern = Regexp.new(/^([0-9]+([.][0-9]+)?(ns|us|µs|ms|s|m|h))+$/)
      if ttl !~ pattern
        fail ArgumentError, "invalid value for \"ttl\", must conform to the pattern #{pattern}."
      end
