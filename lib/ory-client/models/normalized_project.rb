...
module OryClient
  class NormalizedProject < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OryClient::NormalizedProject`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @hosts.length < 1
        invalid_properties.push('invalid value for "hosts", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @hosts.length < 1
    ...
    # Custom attribute writer method with validation
    # @param [Object] current_revision Value to be assigned
    def current_revision=(current_revision)
      if current_revision.nil?
        fail ArgumentError, 'current_revision cannot be nil'
      end
    ...
    def hosts=(hosts)
      ...
      if hosts.length < 1
        fail ArgumentError, 'invalid value for "hosts", number of items must be greater than or equal to 1.'
      end
